...
if not st.session_state.accepted_disclaimer:
    ...
    if st.button("✅ I Understand and Agree", key="agree_btn"):
...
@st.cache_resource
def get_model():
    # Use absolute path based on current file location to ensure it works anywhere (e.g., Streamlit Cloud)
    base_dir = os.path.dirname(os.path.abspath(__file__))
    model_path = os.path.join(base_dir, "model", "lstm_model.h5")
    
    try:
        if not os.path.exists(model_path):
            # Fallback to local relative path just in case
            model_path = "model/lstm_model.h5"
            
        if not os.path.exists(model_path):
            st.error("Model file not found. Please ensure 'lstm_model.h5' exists in the 'model' directory.")
            return None
        ...
        st.info("Suggestion: Ensure TensorFlow/Keras is correctly specified in your 'requirements.txt' and is compatible with your model.")
        return None

lstm_model = get_model()

if lstm_model is None:
    st.warning("App paused. Fix the model loading issue to proceed.")
    st.stop()
...
if st.button("🚀 Predict Next Day Price", key="predict_btn"):
    ...
    if len(scaled_data) >= lookback:
        ...
        if signal == "BUY":
            ...
            st.info("Uptrend detected (Predicted price higher than current price)")
        else:
            st.error("🔴 SELL Signal")
            st.warning("Downtrend detected (Predicted price lower than current price)")
...
if st.button("🔄 Fetch Today's Actual Price", key="fetch_price_btn"):
